...
const Quiz = () => {
  ...
  const setAnswers = (thisAnswer) => {
    const fetchAnswer = userAnswers.find(
      (quizAnswer) => quizAnswer.questionId === thisAnswer.questionId
    );
    if (fetchAnswer) {
  ...
  const submitQuizHandler = async (event) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: `Your total score is ${correctCount} out of ${quizInfo.totalQuestions}`,
  ...
  return (
    <div>
      ...
      <form onSubmit={submitQuizHandler}>
        {loading ? (
          <span style={{ margin: "3rem" }}>{spinner}</span>
        ) : (
          <section>
